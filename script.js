...
class MazeModel{
    ...
    createMaze() {
       ...
       let unvisited=rows*cols-2; // total number of cells in the grid - minus start and goal cells
       ...
       //create grid with all walls set to true
        ...
        let visited=new Set();
        visited.add(this.maze.rows[this.start.row][this.start.col]);
        visited.add(this.maze.rows[this.goal.row][this.goal.col]);
        while(unvisited>0){
    ...
    getUnvisitedNeighbors(neighbors,visited){
        ...
        for(let i=0;i<neighbors.length;i++){
            if(!visited.has(neighbors[i])){
                unvisitedNeighbors.push(cell[i]);
            }
        }
